...
Analysis section in CDT allows to conduct different analyses of climate data. You can compute different statistics and show them in tabular form, show them graphically in plots and also in maps. For temperature data, you can compute the mean and range for different time periods. You can also compute potential evapotranspiration and water balance. Further, you can compute climatologies for different time frequency data. Climatologies can also be plotted or shown in maps wherein these are computed for different stations.  Under spatial analysis, you can compute different statistics aggregated at either monthly, or seasonal, or annual scale, or all of them according to your specification. In addition, time series graphs such as line chart, bar plot, probability of exceedance, line chart with ENSO years, bar plot with ENSO years and probability of exceedance for ENSO, and anomaly plot can also be computed. All of these are computed for separate stations and using CDT, you can explore any of these statistics or graph for a single station selected by you. For rainfall, you can define rainy/wet days and generate different charts for a season defined by you. Line chart, bar plot, probability of exceedance and anomaly plot can be generated using features found under Daily Rainfall Analysis. Seasonal rainfall can also be plotted in map. You can also calculate rainy season onset, rainy season cessation and rainy season length. Using the features available under Rainy Season Analysis, you can also compute many other statistics associated with rainy season such as longest dry spell, number of rainy days, maximum daily rain, total rainfall when rainfall is greater than 95th percentile etc. CDT also allows you to get different climate extreme indices for temperature and rainfall as found in the Climdex website (Indices, 2021). CDT also allows you to compute drought indices such as standardized precipitation index (SPI), standardized precipitation evapotranspiration index (SPEI), and deciles.

## Potential evapotranspiration 

According to Rosenberg, potential evapotranspiration (PET) is “the evaporation from an extended surface of a short green crop which fully shades the ground, exerts little or negligible resistance to the flow of water, and is always well supplied with water. Potential evapotranspiration cannot exceed free water evaporation under the same weather conditions” (ScienceDirect, 2020).

There are many different methods for calculating PET. CDT allows you to use 2 methods for PET which are Hargreaves and modified Hargreaves method.  
...
$$
E_{rc} = 0.0022*R_A*\delta'^{0.5}_T*(T + 17.8)
$$

Where:
* R~A~ = mean extra-terrestrial radiation (mm/day), 
* $\delta'_T$´ = temperature difference = mean monthly maximum temperature - mean monthly minimum temperature for the month of interest ($^{0}C$)
* T = mean air temperature ($^{0}C$)
...
3. Click **Southern Hemisphere: year starting on July**.
...
3. Tick **Southern Hemisphere: year starting on July** if ….
4. Tick **Use bootstrap sampling** when you …
...
3. If you already have calculated SPI dataset, tick this field **Monitoring: update SPI dataset**. You can also specify the date range for which you want to update by clicking on **Set Date Range**. 
...
4.	Click **Visualizing time-scales** to … 
...
3. If you already have calculated decile, tick this field **Monitoring: update Decile dataset**. You can also specify the date range for which you want to update by clicking on **Set Date Range**. 
...
6. Click **Calculate** to calculate the Decile or update the existing Decile dataset. This will create a new folder **Decile_data** under which there will be 4 sub-directories and 1 file **Decile.rds**.
