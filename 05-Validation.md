...
Similar to precipitation, we can perform all the aforementioned validation tasks using the menu **Validation -> Spatial Points Data (CDT Format) -> Precipitation Data**. 
...
The validation for multiple data sets for temperature is very similar to precipitation and so we don’t provide any description here.
